...
ProductRouter.post('/upload', upload.single('image'), async (req, res) => {
  try {
    ...
    await QuerySql('UPDATE products SET img_path = ? WHERE id = ?', [filePath, id]);
...
ProductRouter.get("/img/:id", async (req, res) => {
  try {
    ...
    const results = await QuerySql("SELECT img_path FROM products WHERE id = ?", [id]);
    if (results.length === 0 || !results[0].img_path) {
      return res.status(404).send("Image not found");
    }
    
    // Construct full path to the file
 const imgPath =path.join(__dirname, "..","..", "uploads", results[0].img_path)


    // ".." if this router file is in a subfolder, adjust according to your structure
    console.log(imgPath)
